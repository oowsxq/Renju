...
class SearchAlgorithm {
     ...
     * @return 估值的结果
     */
    public int expandExperimetnal(Board input_board, int depth, char side, int x, int y, int currentLBound) {
        return expand(input_board, depth, side, x, y, currentLBound, Evaluator.VALUE_MAX);
    }
     ...
     * @return 估值的结果
     */
    public int expandExperimental(Board input_board, int depth, char side, int x, int y,
                      int alpha, int beta) {
        this.board = new Board(input_board);
        this.board.setValue(x, y, side);
        return alphaBetaSearch(depth, side, alpha, beta);
     ...
     * @return 此节点的估值
     */
    private int alphaBetaSearchExperimental(int depth, char side, int alpha, int beta) {
        //TODO
        //如果游戏结束 或到达指定深度 返回当前局面估值
        if (Judgementor.adjustGameOver(board) != null || depth <= 0)
            return evaluator.evaluate(board, side);
